<script lang="ts">
  ...
  import { socialLinks, identity } from '$lib/identity';
  ...
  const navTo = (slug: string) => () => { close(); goto(`/${slug}/`); };
  const copyEmail = () => {
    close();
    try { navigator.clipboard?.writeText(identity.email); }
    catch { window.location.href = `mailto:${identity.email}`; }
  };
...
{#if open}
  <div role="dialog" aria-modal="true" aria-label="Command palette" class="fixed inset-0 z-[60] flex items-start justify-center px-4 pt-[14vh] backdrop-blur-md">
    <button type="button" class="absolute inset-0 cursor-default bg-ink-950/60" aria-label="Close palette" on:click={close}></button>
    ...
    <div class="relative z-10 w-full max-w-xl overflow-hidden rounded-2xl border border-ink-600/70 bg-ink-900/95 shadow-[var(--shadow-neon-cyan)] ring-1 ring-neon-cyan/10">
      <div class="flex items-center gap-3 border-b border-ink-600/60 px-4 py-3">
        ...
        <input
          bind:this={inputEl}
          bind:value={query}
          placeholder="Type a post title — or 'github', 'portfolio'…"
